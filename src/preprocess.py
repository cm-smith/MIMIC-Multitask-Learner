...
# Create a PROCESSED_DIR subdirectory within ROOT
ROOT = "./data/"
PROCESSED_DIR = 'mapped_elements/'
...
class ParseItemID(object):
    ...
    def __init__(self):
        ...
        self.feature_names = [
                ...
                'atypical lymphocytes', 'bands', 'basophils', 'eosinophils',
                'neutrophils', 'lymphocytes', 'monocytes', 'polymorphonuclear leukocytes', 
                'temperature (F)', 'heart rate', 'respiratory rate', 'systolic', 'diastolic',
                'pulse oximetry', 'troponin', 'HDL', 'LDL', 'BUN', 'INR', 'PTT', 'PT',
                'triglycerides', 'creatinine', 'glucose', 'sodium', 'potassium',
                'chloride', 'bicarbonate', 'blood culture', 'urine culture',
                'surface culture', 'sputum culture', 'wound culture', 
                'Inspired O2 Fraction', 'central venous pressure', 
        ...
        self.features = ['$^RBC(?! waste)', '$.*wbc(?!.*apache)', '$^platelet(?!.*intake)', 
                         ...
                         'Differential-Atyps', 'Differential-Bands', 'Differential-Basos',
                         'Differential-Eos', 'Differential-Neuts', 'Differential-Lymphs',
                         'Differential-Monos', 'Differential-Polys', 'temperature f',
                         'heart rate', 'respiratory rate', 'systolic', 'diastolic', 
                         'oxymetry(?! )', 'troponin', 'HDL', 'LDL', '$^bun(?!.*apache)',
                         'INR', 'PTT',  
                         ...
                         'surface culture', 'sputum culture', 'wound culture',
                         'Inspired O2 Fraction', '$Central Venous Pressure(?! )',
                         'PEEP set', 'tidal volume \(set\)', 'anion gap', 'daily weight',
                         'tobacco', 'diabetes', 'CV - past']                        
...
class MimicParser(object):
    ...
    def reduce_total(self, filepath):
        ...
        for i, df_chunk in enumerate(pd.read_csv(filepath, iterator=True, chunksize=chunksize)):
            ...
            if i == 0:
                df.to_csv(ROOT+PROCESSED_DIR+'CHARTEVENTS_reduced.csv', index=False,
                          columns=columns)
                print(i)
            else:
                df.to_csv(ROOT+PROCESSED_DIR+'CHARTEVENTS_reduced.csv', index=False,
        ...
        if low_memory==False:
            for i in range(len(buckets)):
                for i,chunk in enumerate(pd.read_csv(filename, iterator=True,
                    ...
                    sliced.to_csv(ROOT + PROCESSED_DIR + 'shard_{0}.csv'.format(i), index=False)
        else:
            for i in range(len(buckets)):
                with open(filename, 'r') as chartevents:
                    chartevents.seek(0)
                    csvreader = csv.reader(chartevents)
                    with open(ROOT+PROCESSED_DIR+'shard_{0}.csv'.format(i), 'w') as shard_writer:
    ...
    def create_day_blocks(self, file_name):
        ...
        print("DAY BLOCKS: Creating shard 1/5")
        ...
        print("DAY BLOCKS: Creating shard 4/5")
        ...
        print("DAY BLOCKS: Creating shard 5/5")
        ...
        print("DAY BLOCKS: Combining shards and post-processing")
...
#                df2[i] = df2[i].where(df2[i] > df2[i].quantile(.875)).fillna(df2[i].median())
        ...
        print("DAY BLOCKS: Completed data processing")
    ...
    def add_patient_columns(self, file_name):
        ...
        df = pd.read_csv(ROOT+'PATIENTS.csv')
    ...
    def clean_prescriptions(self, file_name):
        ...
        pid.prescriptions.to_csv(ROOT+PROCESSED_DIR+'PRESCRIPTIONS_reduced.csv', index=False)
        
    def add_prescriptions(self, file_name):
       
        df_file = pd.read_csv(file_name)
        with open(ROOT+'PRESCRIPTIONS_reduced.csv', 'r') as f:
            csvreader = csv.reader(f)
            with open(ROOT+'PRESCRIPTIONS_reduced_byday.csv', 'w') as g:
        ...
        df = pd.read_csv(ROOT+PROCESSED_DIR+'PRESCRIPTIONS_reduced_byday.csv')
    ...
    def add_icd_infect(self, file_name):
        ...
        df_icd = pd.read_csv(ROOT+'PROCEDURES_ICD.csv')
        df_micro = pd.read_csv(ROOT+'MICROBIOLOGYEVENTS.csv')
    ...
    def add_notes(self, file_name):
        df = pd.read_csv(ROOT+'NOTEEVENTS.csv')
...
if __name__ == '__main__':
    ...
    mp.create_day_blocks(ROOT+ PROCESSED_DIR + FILE_STR + '.csv')
    mp.add_admissions_columns(ROOT + PROCESSED_DIR + FILE_STR + '_24_hour_blocks.csv')
    mp.add_patient_columns(ROOT + PROCESSED_DIR + FILE_STR + '_24_hour_blocks_plus_admissions.csv')
    mp.clean_prescriptions(ROOT + PROCESSED_DIR + FILE_STR + 
